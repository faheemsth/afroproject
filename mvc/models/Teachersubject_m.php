...
class Teachersubject_m extends CI_Model {
	...
	public function teacherSubject() {
		...
		$this->db->from('subjectteacher')->where(array('teacherID' => $teacherID))->order_by($this->_order_by);
	...
	public function teacherSubject_by_array($array=NULL) {
		$schoolyearID = $this->session->userdata('defaultschoolyearID');
		$teacherID = $this->session->userdata('loginuserID');
		...
		$this->db->from('subjectteacher')->where(array('teacherID' => $teacherID, 'sectionID' => $array['sectionID']))->order_by($this->_order_by);
		else
		$this->db->from('subjectteacher')->where(array('teacherID' => $teacherID))->order_by($this->_order_by);
	...
	public function get_order_by_teacher_subject($array = NULL) {
		$subjectArray = $this->teacherSubject_by_array($array);
