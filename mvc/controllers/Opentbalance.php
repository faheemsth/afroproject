...
class Opentbalance extends Admin_Controller{	
	...
	public function index() {
		...
		$this->data["subview"] = "report/opentbalance/BalanceFeesReportView";
					...
					if ($rendertype=="download") {
					 ...
					 //    fclose($file);
					  ...
					  //  exit();
					    ...
					    $retArray['render'] = $this->load->view('report/opentbalance/BalanceFeesReport', $this->data, true);
						...
						// $retArray['status'] = TRUE;
						// echo json_encode($retArray);
						//$this->load->view('report/opentbalance/BalanceFeesReport', $this->data);
						$this->data["subview"] = "report/opentbalance/BalanceFeesReport";
						$this->load->view('_layout_main', $this->data);
	...
	public function pdf() {
		if(permissionChecker('balancefeesreport')) { 
			...
			if((int)($classesID >= 0) || (int)($sectionID >= 0) || (int)($studentID >= 0)) {
				...
				$this->reportPDF('balancefeesreport.css', $this->data, 'report/opentbalance/BalanceFeesReportPDF');
			} else {
		...
		if(permissionChecker('balancefeesreport')) { 
			if($_POST) {
			    ...
			    if ($this->form_validation->run() == FALSE) {
					...
					$this->reportSendToMail('balancefeesreport.css', $this->data, 'report/opentbalance/BalanceFeesReportPDF', $to, $subject, $message);
					$retArray['status'] = TRUE;
