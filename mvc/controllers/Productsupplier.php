    ...
    public function index() {
        ...
        if (isset($_GET)) {
            ...
            if (isset($_GET['company']) && !empty($_GET['company'])) {
                $this->data['company'] = str_replace(' ', '', $_GET['company']);
                $filter['productsupplier.productsuppliercompanyname LIKE'] = '%' . str_replace(' ', '', $_GET['company']) . '%';
            } else {
                $this->data["company"] = "";
            }
            
            if (isset($_GET['supplier_name']) && !empty($_GET['supplier_name'])) {
                $this->data['supplier_name'] = str_replace(' ', '', $_GET['supplier_name']);
                $filter['productsupplier.productsuppliername LIKE'] = '%' . str_replace(' ', '', $_GET['supplier_name']) . '%';
            } else {
                $this->data["supplier_name"] = "";
            }

            if (isset($_GET['email']) && !empty($_GET['email'])) {
                $this->data['email'] = str_replace(' ', '', $_GET['email']);
                $filter['productsupplier.productsupplieremail LIKE'] = '%' . str_replace(' ', '', $_GET['email']) . '%';
            } else {
                $this->data["email"] = "";
            }

            if (isset($_GET['phone']) && !empty($_GET['phone'])) {
                $this->data['phone'] = str_replace(' ', '', $_GET['phone']);
                $filter['productsupplier.productsupplierphone LIKE'] = '%' . str_replace(' ', '', $_GET['phone']) . '%';
            } else {
                $this->data["phone"] = "";
            }
        ...
        $this->data['suppliers'] = $this->productsupplier_m->get_order_by_productsupplier($filter);
        ...
        if($_POST) {
            ...
            if ($this->form_validation->run() == FALSE) {
                ...
                $array = array(
                    ...
                    "productsupplier_strn"            => $this->input->post("productsupplier_strn"),
                    "productsupplier_ntn"             => $this->input->post("productsupplier_ntn"),
                    "productsupplier_bank"            => $this->input->post("productsupplier_bank"),
                    "productsupplier_branch_code"     => $this->input->post("productsupplier_branch_code"),
                    "productsupplier_account_title"   => $this->input->post("productsupplier_account_title"),
                    "productsupplier_account_number"  => $this->input->post("productsupplier_account_number"),
                    "create_date"                     => date("Y-m-d H:i:s"),
                    "modify_date"                     => date("Y-m-d H:i:s"),
                    "create_userID"                   => $this->session->userdata('loginuserID'),
                    "create_usertypeID"               => $this->session->userdata('usertypeID'),
                    "supplier_quotation"          => $this->input->post("productsupplier_quotation")
                );
