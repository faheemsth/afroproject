...
*/
		...
		if(($this->data['siteinfos']->school_year == $this->session->userdata('defaultschoolyearID') || $this->session->userdata('usertypeID') == 1)) {
	        ...
	        if($_POST) {
	            ...
	            if ($this->form_validation->run() == FALSE) {
					...
					$total_marks_arr = [];
					foreach($this->data['markpercentages'] as $per){
						$filter['MarkPercentageID'] =  $per->markpercentageID;
						$filter['classID'] = $_POST['classesID'];
						$filter['ExamID'] =  $_POST['examID'];
						$filter['SubjectID'] = $_POST['subjectID'];
						$filter['SectionID'] = $_POST['sectionID'];
						$total_marks = $this->markdistribution_m->get_single_markdistribution($filter);
						$total_marks_arr[$per->markpercentageID] = $total_marks;
					}

					$this->data['total_marks'] = $total_marks_arr;
	...
	public function addTotalMarks() {

		if(!empty($_POST['totalmarks'])){
			foreach($_POST['totalmarks'] as $key => $total_marks){
				if($key == 0 && empty($total_marks))
				continue;

				$filter['MarkPercentageID'] =  $key;
				$filter['ClassID'] = $_POST['classID'];
				$filter['ExamID'] =  $_POST['examID'];
				$filter['SubjectID'] = $_POST['subjectID'];
				$filter['SectionID'] = $_POST['sectionID'];
				$marks_exist = $this->markdistribution_m->get_single_markdistribution($filter);
				
				if(isset($marks_exist->MarkDistirbutionID)){
					$filter['total_marks'] = $total_marks;
					$this->markdistribution_m->update_markdistribution($filter, $marks_exist->MarkDistirbutionID);
					unset($filter['total_marks']);
				}else{
					$filter['total_marks'] = $total_marks;
					$this->markdistribution_m->insert_markdistribution($filter);
				}
	...
	public function subjectcall() {
		...
		if((int)$id) {
			$allsubject = $this->subject_m->get_order_by_subject(array("classesID" => $id, 'sectionID' => $sectionID));
