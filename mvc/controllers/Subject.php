...
*/
		...
		if((int)$id && (int)$url) {
			...
			if(isset($fetchClass[$url])) {
				...
				if(customCompute($this->data['subject'])) {
					$this->data['set'] = $url;
					$this->data['sections'] = $this->section_m->general_get_order_by_section(array("classesID" =>$this->data['subject']->classesID));
					if($_POST) {
						$rules = $this->rules();
						$this->form_validation->set_rules($rules);
						if ($this->form_validation->run() == FALSE) {
	...
	public function subjectStatus(){
		...
		$status = $this->input->post('subjectStatus');

		$this->subject_m->update_subject([
			'status' => $status
		], $subjectID);
