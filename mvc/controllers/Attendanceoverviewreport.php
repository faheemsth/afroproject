...
class Attendanceoverviewreport extends Admin_Controller {
	...
	public function index() {
		if($this->session->userdata("usertype") == 'Admin' ||  $this->session->userdata("usertype") == 'OSA' ){
	...
	public function getSubject() {
		//die($this->session->userdata('defaultschoolyearID'));
		$sectionID = $this->input->post('sectionID');
		if((int)$sectionID) {
			//get_subject_byteacher_wherein_subjectID_array
			
			if($this->session->userdata("usertype") == 'Admin' || $this->session->userdata("usertype") == 'OSA'){
				$subjects = $this->subject_m->general_get_order_by_subject(array('sectionID' => $sectionID));
			}else if($this->session->userdata("usertype") == 'Teacher'){
				$subjects = $this->subject_m->get_subject_byteacher_wherein_subjectID_array(array('subjectteacher.sectionID' => $sectionID, 'subjectteacher.teacherID' => $this->session->userdata('loginuserID')));
			}else if($this->session->userdata("usertype") == 'Student'){
				$subjects = $this->subject_m->get_subject_bystudent_wherein_subjectID_array(['student.studentID' => $this->session->userdata('loginuserID'), 'subjectenrollment.sectionID' => $sectionID]);
			}
	...
	public function getStudent() {
		...
		if((int)$usertype && (int)$classesID && (int)$sectionID) {
			...
			if($this->session->userdata("usertype") == 'Admin' || $this->session->userdata("usertype") == 'Teacher' || $this->session->userdata("usertype") == 'OSA' ){
				if($usertype == 1) {
			...
			if($_POST) {
				...
				if ($this->form_validation->run() == FALSE) {
					...
					if($this->session->userdata("usertype") == 'Student'){
						...
						$_POST['userID'] =  $this->session->userdata('loginuserID');
					}
