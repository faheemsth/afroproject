    ...
    */
    ...
    public function index()
    {
        ...
        if ($usertypeID == 3) {
            ...
            $this->data["maininvoice_type_v"]   = "invoice";
            ...
            if ($_GET) {
                ...
                if ($nameSearch != "") {
                    $array["nameSearch"]        = $nameSearch;
                    $this->data["nameSearch"]   = $nameSearch;
                }
                $maininvoice_type_v = $_GET["maininvoice_type_v"];
                ...
                if ($maininvoicestatus != "") {
                    $array["maininvoicestatus"]         = $maininvoicestatus;
                    $this->data["maininvoicestatus"]    = $maininvoicestatus;
                }
                ...
                $count = $this->maininvoice_m->get_maininvoice_with_studentrelation_by_array(0, 0, $schoolyearID, "count", $array);
                ...
                if ($rendertype == 'download') {
                    ...
                    $allinvoices = $this->maininvoice_m->get_maininvoice_with_studentrelation_by_array(
                        $count,
                        0,
                        $schoolyearID,
                        "data",
                    ...
                    $this->data["maininvoices"] = $this->maininvoice_m->get_maininvoice_with_studentrelation_by_array(
                        $returns["page"],
                        $returns["segment"],
                        $schoolyearID,
                        "data",
    ...
    public function index_old()
    {
        ...
        if ($usertypeID == 3) {
            $username = $this->session->userdata("username");
            $student = $this->student_m->get_single_student([
                "username" => $username,
            ]);
            if (customCompute($student)) {
                $this->data["maininvoices"] = $this->maininvoice_m->get_maininvoice_with_studentrelation_by_studentID($student->studentID, $schoolyearID);
    ...
    public function unique_paymentmethod()
    {
        if ($this->input->post("paymentmethodID") === "0") {
            ...
            if (
                $this->input->post("paymentmethodID") == "Cash" ||
                                            ...
                                            if (customCompute($this->data["invoices"])) {
                                                ...
                                                foreach ($this->data["invoices"] as $key => $invoice) {
                                                    if ($invoice->paidstatus != 2) {
                                                        ...
                                                        if (
                                                            (float) $this->input->post("weaver_" . $invoice->invoiceID) > (float) 0 ||
        ...
        if (permissionChecker("invoice_view")) {
            ...
            if ((int) $globalpaymentID && (int) $maininvoiceID) {
                $globalpayment = $this->globalpayment_m->get_single_globalpayment(
                    ["globalpaymentID" => $globalpaymentID, "schoolyearID" => $schoolyearID,]
                );
                $maininvoice = $this->maininvoice_m->get_single_maininvoice(["maininvoiceID" => $maininvoiceID, "maininvoiceschoolyearID" => $schoolyearID,]);
                if (
                    ...
                    $f = false;
                    if ($usertypeID == 3) {
                        $getstudent = $this->studentrelation_m->get_single_studentrelation(["srstudentID" => $globalpayment->studentID, "srschoolyearID" =>    $globalpayment->schoolyearID,]);
                        if (customCompute($getstudent)) {
                    ...
                    if ($f) {
                        $studentrelation = $this->studentrelation_m->get_single_studentrelation(["srstudentID" => $globalpayment->studentID, "srschoolyearID" =>    $globalpayment->schoolyearID,]);
                        if (customCompute($studentrelation)) {
                            ...
                            $this->data["weaverandfines"] = pluck(
                                $this->weaverandfine_m->get_order_by_weaverandfine(["globalpaymentID" => $globalpaymentID]),
                                "obj",
                                "paymentID"
                            );
                ...
                if (
                    ...
                    if ($f) {
                        ...
                        if (customCompute($studentrelation)) {
                            ...
                            $this->data["student"] = $this->student_m->get_single_student(["studentID" => $globalpayment->studentID,]);
                            $this->data["invoices"] = pluck(
                                $this->invoice_m->get_order_by_invoice(["maininvoiceID" => $maininvoiceID,]),
                                "obj",
                                "invoiceID"
                            );
                            $this->payment_m->order_payment("paymentID", "asc");
                        ...
                        if (
                            ...
                            if ($f) {
                                ...
                                if (customCompute($studentrelation)) {
                                    ...
                                    $this->data["student"] = $this->student_m->get_single_student(["studentID" =>        $globalpayment->studentID,]);
                                    $this->data["invoices"] = pluck(
                                        $this->invoice_m->get_order_by_invoice([
                                            "maininvoiceID" => $maininvoiceID,
                                        ]),
                                        "obj",
                                        "invoiceID"
                                    );
                                    $this->payment_m->order_payment(
                                        "paymentID",
                                        "asc"
                                    );
                                    $this->data["payments"] = $this->payment_m->get_order_by_payment(["globalpaymentID" => $globalpaymentID,]);
                                    $this->data["weaverandfines"] = pluck(
                                        $this->weaverandfine_m->get_order_by_weaverandfine(
                                            [
                                                "globalpaymentID" => $globalpaymentID,
                                            ]
                                        ),
    ...
    public function invoice_account_import()
    {
        ...
        if (((int) $studentID || $studentID == 0) && (int) $classesID) {
                ...
                if (customCompute($invoiceMainArray)) {
                    ...
                    if ($lastID >= $firstID) {
                        ...
                        for ($i = $firstID; $i <= $lastID; $i++) {
                            ...
                            $paymentHistoryArray[] = [
                                "paymenttype" => ucfirst($this->input->post("paymentmethodID")),    // 'paymentamount' =>  $feetypeitem->paidamount
                        ...
                        if ($invoicelastID >= $invoicefirstID) {
                            ...
                            for (
                                ...
                                $paymentArray[] = [
                                    ...
                                    "paymentamount" => isset($paymentHistoryArray[$k]["paymentamount"])
                                    ...
                                    "transactionID" =>    "CASHANDCHEQUE" . random19(),
                                    "globalpaymentID" => isset($globalPaymentIDArray[$invoiceArray[$k]["studentID"]])
                                        ? $globalPaymentIDArray[$invoiceArray[$k]["studentID"]]
                                        : 0,
                                ];
                                $k++;
                            }
            ...
            $retArray["error"] = [
                "classstudent" => "Class and Student not found.",
            ];
            echo json_encode($retArray);
            exit();
        }
                ...
                if ($_POST) {
                    ...
                    if ($this->form_validation->run() == false) {
                        ...
                        if (isset($_FILES["csvFile"])) {
                            ...
                            $config['allowed_types'] = 'text/plain|text/csv|csv|pdf|doc|docx';
                            ...
                            if (!$this->upload->do_upload("csvFile")) {
                                ...
                                $retArray["error"] =   $this->upload->display_errors();;
                                $retArray["status"] = false;
                                echo json_encode($retArray);
                            } else {
                        ...
                        if (((int) $studentID || $studentID == 0) &&
                            ...
                            if ($studentID != 0) {
                                ...
                                if ($invoice_type_v == "hostel_fee") {
                                    $studentArrays["hostel"] = 1;
                                }
                                if ($invoice_type_v == "Transport_fee") {
                                    $studentArrays["transport"] = 1;
                                }
                            ...
                            if (customCompute($getstudents)) {
                                ...
                                if ($this->data["siteinfos"]->attendance_fine_feetypeID == $this->input->post("feess_type")) {
                                    ...
                                    ///////////////////////Added by Sanaullah
                                    if ($_POST['maininvoice_type_v'] == 'other_charges' && $_POST['feess_type'] == 22) {
                                        $attendance_start_date = $_POST['attendance_start_date'];
                                        $attendance_end_date = $_POST['attendance_end_date'];
                                        $allMonths = get_month_and_year_using_two_date($attendance_start_date, $attendance_end_date);
                                    } else {
                                    ...
                                    for ($y = 1; $y <= $instcounter; $y++) {
                                        ...
                                        if ($invoice_type_v == "other_charges") {
                                            ...
                                            if ($this->data['siteinfos']->attendance_fine_feetypeID == $feess_type) {
                                                ...
                                                $total_fee  = (ceil($student_attendance_fine[$getstudent->srstudentID] / 1.3333));
                                                $net_fee    = (ceil($student_attendance_fine[$getstudent->srstudentID] / 1.3333));
                                                ...
                                                if (is_null($getstudent->enroll_date)) {
                                                    ...
                                                    $now =   strtotime($getstudent->enroll_date);
                                                }
                                                ...
                                                if ($fine_type == '1') {
                                                    //day wise fine 
                                                    $total_fee  = $this->input->post("amount") * $ndays;
                                                    $net_fee    = $this->input->post("amount") * $ndays;
                                                } else {
                                                    $total_fee  = $this->input->post("amount");
                                                    $net_fee    = $this->input->post("amount");
                                                }
                                        ...
                                        for ($i = $firstID; $i <= $lastID; $i++) {
                                            ...
                                            if ($invoice_type_v == 'invoice') {
                                                ...
                                                $ledger_array[] = [
                                                    ...
                                                    "net_fee"       => isset($invoiceMainArray[$j]["maininvoicenet_fee"]) ? $invoiceMainArray[$j]["maininvoicenet_fee"] : 0,
                                                    "refrence_no"   => $ref_array[date('ym', strtotime($invoiceMainArray[$j]["maininvoicedate"]))],
                                                    'description'   => $feetype . ' fee for semster ' . $invoiceMainArray[$j]['maininvoicestd_srid'] . $remarks,
                                                    "accounts_reg"  => $reg_no[$invoiceMainArray[$j]["maininvoicestudentID"]],
        ...
        if (
            ...
            if (
                ...
                if ($_POST) {
                    ...
                    // $num = (int)$ref_nos + 1;
                    // $num = sprintf("%02d", $num);
                    $classes = $this->classes_m->general_get_single_classes([
                        "classesID" => $classesID,
                    ]);
                    $classes_numeric = $classes->classes_numeric;
                    if ($studentID > 0) {
                    ...
                    if (customCompute($getstudents)) {
                        ...
                        foreach ($getstudents as $key => $getstudent) {
                            ...
                            for ($y = 1; $y <= $instcounter; $y++) {
                                ...
                                if (
                                    $total_hostel_fee - $hostel_fee_discount >
                                    0
                                ) {
                                    # code...
                            ...
                            for ($y = 1; $y <= $instcounter; $y++) {
                                ...
                                if (
                                    ...
                                    $invoiceMainArray[] = [
                                        ...
                                        "maininvoicetotal_fee" => ceil(
                                            $total_transport_fee / $instcounter
                                        ),
                                        "maininvoice_discount" => ceil(
                                            $transport_fee_discount /
                                                $instcounter
                                        ),
                    ...
                    if ((int) $editID) {
                        ...
                        foreach ($feetypeitems as $fb) {
                            ...
                            $journal_items_ar_cr_up  = array(
                                'credit'        =>  $net_amount,
                                'description'   =>  $get_feetype->feetypes . ' fee for semster ' . $old_invoice_data->maininvoicestd_srid . $this->input->post("remarks"),
                                'updated_at'    =>  date('Y-m-d h:i:s'),
                            );
                            ...
                            $student_ledgers_ar_up_cr  = array(
                                'credit'                   =>  $net_amount,
                                'balance'                  =>  $net_amount,
                                'narration'   =>  $get_feetype->feetypes . ' fee for semster ' . $old_invoice_data->maininvoicestd_srid . $this->input->post("remarks"),
                                'updated_at'               =>  date('Y-m-d h:i:s')
                            );
                    ...
                    if (
                        ...
                        foreach ($invoiceitems[$maininvoice->maininvoiceID]
                            ...
                            if (isset($retArray["grandtotal"][$maininvoice->maininvoiceID])) {
                                $retArray["grandtotal"][$maininvoice->maininvoiceID] =
                            ...
                            if (isset($retArray["totalamount"][$maininvoice->maininvoiceID])) {
                                $retArray["totalamount"][$maininvoice->maininvoiceID] =
                            ...
                            if (isset($retArray["totaldiscount"][$maininvoice->maininvoiceID])) {
                                $retArray["totaldiscount"][$maininvoice->maininvoiceID] =
                                    $retArray["totaldiscount"][$maininvoice->maininvoiceID] +
                                    ($invoiceitem->amount / 100) *
                                    $invoiceitem->discount;
                            } else {
                                ...
                                if (customCompute($paymentitems[$invoiceitem->invoiceID])) {
                                    foreach ($paymentitems[$invoiceitem->invoiceID]
                                        ...
                                        if (isset($retArray["totalpayment"][$maininvoice->maininvoiceID])) {
                                            $retArray["totalpayment"][$maininvoice->maininvoiceID] =
                                ...
                                if (customCompute($weaverandfineitems[$invoiceitem->invoiceID])) {
                                    foreach ($weaverandfineitems[$invoiceitem->invoiceID]
                                        ...
                                        if (isset($retArray["totalweaver"][$maininvoice->maininvoiceID])) {
                                            $retArray["totalweaver"][$maininvoice->maininvoiceID] =
                                        ...
                                        if (isset($retArray["totalfine"][$maininvoice->maininvoiceID])) {
                                            $retArray["totalfine"][$maininvoice->maininvoiceID] =
                ...
                if (customCompute($invoiceitems[$maininvoice->maininvoiceID])) {
                    foreach ($invoiceitems[$maininvoice->maininvoiceID]
                        ...
                        if (isset($retArray["totaldiscount"][$invoiceitem->invoiceID])) {
                            $retArray["totaldiscount"][$invoiceitem->invoiceID] =
                                $retArray["totaldiscount"][$invoiceitem->invoiceID] +
                                ($invoiceitem->amount / 100) *
                                $invoiceitem->discount;
                        } else {
                        ...
                        if (isset($paymentitems[$invoiceitem->invoiceID])) {
                            if (customCompute($paymentitems[$invoiceitem->invoiceID])) {
                                foreach ($paymentitems[$invoiceitem->invoiceID]
                                    as $paymentitem) {
                                    if (isset(
                                        $retArray["totalpayment"][$paymentitem->invoiceID]
                                    )) {
                                        $retArray["totalpayment"][$paymentitem->invoiceID] =
                        ...
                        if (isset($weaverandfineitems[$invoiceitem->invoiceID])) {
                            if (customCompute($weaverandfineitems[$invoiceitem->invoiceID])) {
                                foreach ($weaverandfineitems[$invoiceitem->invoiceID]
                                    as $weaverandfineitem) {
                                    if (isset(
                                        $retArray["totalweaver"][$weaverandfineitem->invoiceID]
                                    )) {
                                        $retArray["totalweaver"][$weaverandfineitem->invoiceID] =
                                    ...
                                    if (isset(
                                        $retArray["totalfine"][$weaverandfineitem->invoiceID]
                                    )) {
                                        $retArray["totalfine"][$weaverandfineitem->invoiceID] =
                    ...
                    foreach ($invoiceitems[$maininvoice->maininvoiceID]
                        ...
                        if (isset($weaverandfineitems[$invoiceitem->invoiceID])) {
                            if (customCompute($weaverandfineitems[$invoiceitem->invoiceID])) {
                                foreach ($weaverandfineitems[$invoiceitem->invoiceID]
                                    as $weaverandfineitem) {
                                    $retArray["globalpayments"][$weaverandfineitem->globalpaymentID][$weaverandfineitem->paymentID]["weaver"] =
                                        $weaverandfineitem->weaver;

                                    $retArray["globalpayments"][$weaverandfineitem->globalpaymentID][$weaverandfineitem->paymentID]["fine"] =
                                        $weaverandfineitem->fine;
                                }
                            }
                        }
            ...
            if (
                ...
                if (customCompute($maininvoice)) {
                    ...
                    $globalpayments = pluck(
                        $this->globalpayment_m->get_order_by_globalpayment([
                            "studentID" => $maininvoice->maininvoicestudentID,
                        ]),
                        "obj",
                        "globalpaymentID"
                    ...
                    if (customCompute($invoices)) {
                        foreach ($invoices as $invoice) {
                            $payments = $this->payment_m->get_orderr_by_payment([
                                "invoiceID" => $invoice->invoiceID,
                                "studentID" =>    $maininvoice->maininvoicestudentID,
                            ]);
                            ...
                            if (customCompute($payments)) {
                                foreach ($payments as $payment) {
                                    if (isset(
                                        $globalpayments[$payment->globalpaymentID]
                                    )) {
                    ...
                    if (customCompute($globalpaymentobjects)) {
                        foreach ($globalpaymentobjects
                            ...
                            if (isset($allpayments[$globalpaymentobject->globalpaymentID])) {
                                if (customCompute($allpayments[$globalpaymentobject
                                    ->globalpaymentID])) {
                                    foreach ($allpayments[$globalpaymentobject
                                        ->globalpaymentID]
                                        as $payment) {
                                        if (isset($paymentlists[$globalpaymentobject
                                            ->globalpaymentID])) {
                                            $paymentlists[$globalpaymentobject->globalpaymentID]["paymentamount"] +=
                                    ...
                                    if (isset(
                                        $allweaverandfines[$globalpaymentobject
                                            ->globalpaymentID]
                                    )) {
                                        foreach ($allweaverandfines[$globalpaymentobject
                                            ->globalpaymentID]
                                            as $allweaverandfine) {
                                            if (
                                                ...
                                                if (isset(
                                                    $paymentlists[$globalpaymentobject
                                                        ->globalpaymentID]
                                                )) {
                                                    $paymentlists[$globalpaymentobject->globalpaymentID]["weaveramount"] = $allweaverandfine->weaver;
                                                    $paymentlists[$globalpaymentobject->globalpaymentID]["fineamount"] = $allweaverandfine->fine;
                ...
                if ((int) $globalpaymentID && (int) $maininvoiceID) {
                    ...
                    if (customCompute($globalpayment)) {
                        ...
                        foreach ($payments as $payment) {
                            if (
                                ...
                                if (isset($weaverandfines[$payment->paymentID])) {
                                    $weaverandfineArray[] =
                                        $weaverandfines[$payment->paymentID]->weaverandfineID;
                                }
                            } else {
                                $excType = false;
                                $this->data["subview"] = "error";
                        ...
                        if ($excType) {
                            ...
                            $paymentsum = $this->payment_m->get_where_payment_sum(
                                "paymentamount",
                                "invoiceID",
                                $invoicepluck
                            );
                            ...
                            $payments = pluck(
                                $this->payment_m->get_where_payment_sum(
                                    "paymentamount",
                                    "invoiceID",
                                    $invoicepluck,
                                    "invoiceID"
                            ...
                            $weaverandfines = pluck(
                                $this->weaverandfine_m->get_where_weaverandfine_sum(
                                    ["weaver", "fine"],
                                    "invoiceID",
                                    $invoicepluck,
                                    "invoiceID"
                            ...
                            if (customCompute($invoiceArray)) {
                                $this->invoice_m->update_batch_invoice(
                                    $invoiceArray,
                                    "invoiceID"
                                );
                            }
            ...
            if ($purchaseId) {
                ...
                if (!customCompute($dbTransactionID)) {
                    ...
                    if (customCompute($maininvoice)) {
                        ...
                        if (customCompute($this->data["student"])) {
                            $this->data["invoices"] = $this->invoice_m->get_order_by_invoice([
                                "maininvoiceID" => $params["invoice_id"],
                                "deleted_at" => 1,
                            ]);
                            ...
                            $this->data["invoicepaymentandweaver"] = $this->paymentdue(
                                $maininvoice,
                                $schoolyearID,
                            ...
                            if (customCompute($this->data["invoices"])) {
                                ...
                                foreach ($this->data["invoices"]
                                    ...
                                    if ($invoice->paidstatus != 2) {
                                        ...
                                        $totalPayment = 0;
                                        if (
                                            isset($params["allpost"]["paidamount_" .
                                                $invoice->invoiceID]) &&
                                        ...
                                        if (
                                            isset($params["allpost"]["weaver_" .
                                                $invoice->invoiceID]) &&
                                            $params["allpost"]["weaver_" . $invoice->invoiceID] > 0
                                        ) {
                                            $totalPayment +=
                    ...
                    if (customCompute($this->data["student"])) {
                        ...
                        if (customCompute($this->data["invoices"])) {
                            ...
                            foreach ($this->data["invoices"]
                                ...
                                if ($invoice->paidstatus != 2) {
                                    if (isset(
                                        $invoicepaymentandweaver["totalamount"][$invoice->invoiceID]
                                    )) {
                                        $due =
                                            (float) $invoicepaymentandweaver["totalamount"][$invoice->invoiceID];
                                    ...
                                    if (
                                        ...
                                        $params["allpost"]["paidamount_" . $invoice->invoiceID] > 0
                                    ) {
                                        $totalPayment +=
                                            (float) $params["allpost"]["paidamount_" .
                                                $invoice->invoiceID];
                                    ...
                                    if (
                                        isset(
                                            $params["allpost"]["weaver_" . $invoice->invoiceID]
                                        ) &&
                                        $params["allpost"]["weaver_" . $invoice->invoiceID] > 0
                                    ) {
                                        $totalPayment +=
                                            (float) $params["allpost"]["weaver_" . $invoice->invoiceID];
                                    }
                                    ...
                                    $paymentArray = [
                                        ...
                                        "schoolyearID" => $schoolyearID,
                                        "studentID" => $invoice->studentID,
                                        "paymentamount" =>        $params["allpost"]["paidamount_" .
                                            $invoice->invoiceID] == ""                    ? null
                                            : $params["allpost"]["paidamount_" .
                                                $invoice->invoiceID],
                                        "paymenttype" => ucfirst(
                                            $params["allpost"]["paymentmethodID"]
                                        ),
                                        "paymentdate" => date("Y-m-d"),
                                        "paymentday" => date("d"),
                                        "paymentmonth" => date("m"),
                                    ...
                                    if (
                                        (float) $params["allpost"]["weaver_" . $invoice->invoiceID] > (float) 0 ||
                                        (float) $params["allpost"]["fine_" . $invoice->invoiceID] > (float) 0
                                    ) {
                                        $weaverandfineArray = [
        ...
        if ($api_config["stripe_secret"] == "") {
            ...
            if (
                ...
                try {
                    ...
                    $formData = [
                        "number" => $this->item_data["card_number"],
                                    ...
                                    if (customCompute($this->data["student"])) {
                                        ...
                                        if (customCompute($this->data["invoices"])) {
                                            ...
                                            foreach ($this->data["invoices"]
                                                ...
                                                if ($invoice->paidstatus != 2) {
                                                    if (isset(
                                                        $invoicepaymentandweaver["totalamount"][$invoice
                                                            ->invoiceID]
                                                    )) {
                                                    ...
                                                    $paymentArray = ["invoiceID" =>        $invoice->invoiceID,    "schoolyearID" => $schoolyearID,    "studentID" =>        $invoice->studentID,    "paymentamount" =>        $this->item_data["paidamount_" .
                                                        ...
                                                        : $this
                                                            ->item_data["paidamount_" .
                                                            $invoice->invoiceID],    "paymenttype" => ucfirst(
                                                        $this->item_data["paymentmethodID"]
                                            ...
                                            if (in_array(false, $globalstatus)) {
                                                $this->globalpayment_m->update_globalpayment(
                                                    ["clearancetype" =>                        "partial",],
                                                    $globalLastID
                                                );
                                                $this->maininvoice_m->update_maininvoice(
            ...
            if ($result->state == "success") {
                ...
                if (!customCompute($dbTransactionID)) {
                    ...
                    if (customCompute($maininvoice)) {
                        $this->data["student"] = $this->studentrelation_m->get_single_studentrelation(["srstudentID" =>    $maininvoice->maininvoicestudentID, "srschoolyearID" => $schoolyearID,]);
                        if (customCompute($this->data["student"])) {
                            $this->data["invoices"] = $this->invoice_m->get_order_by_invoice(["maininvoiceID" => $params["invoice_id"], "deleted_at" => 1,]);
    ...
    public function verifyVoguePayPayment($transaction_id)
    {
        ...
        if ($details->total < 1) {
            return json_encode(["state" => "error", "msg" => "Invalid Transaction",]);
        }
        if ($details->status != "Approved") {
            return json_encode(["state" => "error", "msg" => "Transaction {$details->status}",]);
        }
        return json_encode(["state" => "success", "msg" => "Transaction Approved", "details" => $details,]);
    }
