    ...
    */
    ...
    public function add() {
        if(($this->data['siteinfos']->school_year == $this->session->userdata('defaultschoolyearID')) || ($this->session->userdata('usertypeID') == 1) || ($this->session->userdata('defaultschoolyearID') == 5)) {
            ...
            $this->data['productpurchasequintity'] = (pluck($this->productpurchaseitem_m->get_productpurchaseitem_quantity(), 'obj', 'productID'));

            $this->data['productsalequintity'] = (pluck($this->productsaleitem_m->get_productsaleitem_quantity(), 'obj', 'productID'));
            

            $this->data['productsalequintityforedit'] = $this->requisitionitem_m->get_order_by_requisitionitem(array('schoolyearID' => $schoolyearID, 'requisitionID' => $id));
            $this->data['productsaleitems'] = $this->requisitionitem_m->get_order_by_requisitionitem(array('schoolyearID' => $schoolyearID, 'requisitionID' => $id));
            ...
            if(permissionChecker('productsale_add') || permissionChecker('productsale_edit')) {
                if($_POST) {
                    ...
                    if ($this->form_validation->run() == FALSE) {
                        ...
                        $this->requisition_m->update_requisition(array('requisitionstatus' => 1), $this->input->post('requisitionID'));
                        $this->session->set_flashdata('success', $this->lang->line('menu_success'));
                        $retArray['productsaleID'] = $productsaleID;
                        $retArray['status'] = TRUE;
                        $retArray['message'] = 'Success';
                        echo json_encode($retArray);
                        exit;
