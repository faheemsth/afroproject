...
class Attendancemultisubjectreport extends Admin_Controller
{
	...
	public function getSubject()
	{
		...
		if ((int)$sectionID) {
			$subjects = $this->subject_m->general_get_order_by_subject(array('sectionID' => $sectionID, 'status' => 1));
			//echo "<option value='0'>", $this->lang->line("attendanceoverviewreport_please_select"),"</option>";
				...
				if ($this->form_validation->run() == FALSE) {
					...
					if ($usertype == '1') {
						if ($this->data["siteinfos"]->attendance == 'subject') {
							...
							$studentlist 	=	$this->student_m->get_order_by_student($get_st_array, [0, 1]);
	...
	private function xmlData()
	{
		...
		if ($this->data["siteinfos"]->attendance == 'subject') {
			...
			$startdate    = date('d-m-Y', (int)htmlentities(escapeString($this->uri->segment(8))));
			$enddate    = date('d-m-Y', (int)htmlentities(escapeString($this->uri->segment(9))));
		...
		if ((int)$usertype && ((int)$classesID || $classesID >= 0) && ((int)$sectionID || $sectionID >= 0) && ($flag || (int)$subjectID) && ((int)$userID || $userID >= 0) && (int)strtotime($monthID)) {
			...
			redirect('attendancemultisubjectreport');
		}
	}
	...
	private function getMergeCells()
	{
		// Define the columns to merge and their corresponding styles
		$mergeColumns = [
			['start' => 'A', 'end' => 'E'],
			['start' => 'F', 'end' => 'I'],
			['start' => 'J', 'end' => 'M'],
	...
	function mergeColumns($sheet, $startColumn, $endColumn, $startRow, $endRow, $styleArray)
	{
		...
		// Apply the styles to the merged cell
	...
	private function generateXML($data)
	{
		...
		if (customCompute($users)) {
			...
			foreach ($studentlist as $key => $user) {
				...
				$column = 'A';
				$column_count = $key + 1;
				$sheet->setCellValue($column . $row, "$column_count");
				$column++;
				...
				$sheet->setCellValue($column . $row, $parents[$user->parentID]);
				$column++;
				...
				foreach ($subjects as $sub) {
					...
					$total_absent_by_student = $absentCount[$user->studentID][$sub->subjectID];
					$sheet->setCellValue($column . $row, "$total_absent_by_student");
					$column++;
				...
				$absent_per = ($student_percantage / count($subjects));
				$sheet->setCellValue($column . $row, "$absent_per");
				$column++;
	...
	private function getAttendanceRecord()
	{
	}
}
