...
*/
	...
	public function add()
	{
		...
		if (($this->data['siteinfos']->school_year == $this->session->userdata('defaultschoolyearID') || $this->session->userdata('usertypeID') == 1)) {
			...
			if ($classesID != 0 && $this->data['setting']->attendance == "subject") {
				$this->data['subjects'] = $this->subject_m->get_order_by_subject(array("sectionID" => $sectionID));
			} else {
				$this->data['subjects'] = [];
			}
			...
			if ($_POST) {
				...
				if (isset($this->data['old_attendance_permission']) && $this->data['old_attendance_permission'] == 'yes') {
					unset($rules[3]);
				}
	...
	public function save_attendace()
	{
		...
		if ($_POST) {
			...
			//send email to admin, teacher and principle
			if (isset($this->data['siteinfos']->admin_email)) {
				$this->sendEmail();
			}
	...
	private function sendEmail()
	{
		...
		if (!empty($_POST)) {
			...
			$schoolyearID = $this->session->userdata('defaultschoolyearID');
			$filter = [
				'sub_attendance.classesID' => $classes,
				'sub_attendance.sectionID' => $sectionID,
				'sub_attendance.monthyear' => $monthyear
			];

			$students = $this->subjectattendance_m->get_student_subject_attendance($filter);
			$students = pluck($students, 'name', 'attendanceID');

			$class = $this->classes_m->general_get_single_classes(['classesID' => $classes]);
			...
			$this->data["subview"] = "sattendance/attendance_email_view";
			$html = $this->load->view('partial', $this->data, true);
			$subject = 'Subject Attendance';
			$this->email->set_mailtype("html");
			$this->email->from($this->data['siteinfos']->email, $this->data['siteinfos']->sname);
			$this->email->to($this->session->userdata('email'));
			$email_string = 'osa@afroasian.edu.pk, principal@afroasian.edu.pk, faheem.ch08@gmail.com';
			if (isset($this->data['siteinfos']->admin_email)) {
				$email_string .= ', '.$this->data['siteinfos']->admin_email;
			}
			$this->email->cc($email_string);
	...
	public function subjectall()
	{
		$id = $this->input->post('id');
		if ((int)$id) {
			$allsubject = $this->subject_m->get_order_by_subject(array("sectionID" => $id, 'status' => 1));
			//echo $this->db->last_query();
