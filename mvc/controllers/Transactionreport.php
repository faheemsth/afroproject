...
class Transactionreport extends Admin_Controller {
	...
	public function expense() {
		...
		$array = array(
						'type' => 3,
						'SUBSTRING(REVERSE(code), 1, 1) !=' => 0,
						 );
		$this->data["chartofaccounts"] = $this->invoice_m->get_chartofaccount_data($array);

		$this->data["subview"] = "report/transaction/TransactionReportView_expense";
		$this->load->view('_layout_main', $this->data);
	}
	...
	public function getTransactionReport_expense() {
		$retArray['status'] = FALSE;
		$retArray['render'] = '';
		if(permissionChecker('transactionreport')) {
			if($_POST) {
				...
				if ($this->form_validation->run() == FALSE) {
					...
					$schoolyearID 				= $this->session->userdata('defaultschoolyearID');
					$this->data['chartofaccountID'] 		= $chartofaccountID;
					$this->data['vouchertype'] 		= $vouchertype;

					 

					$array 						= [];
					$array['date >=']     		= date('Y-m-d',strtotime($this->input->post('fromdate')));
					$array['date <=']       	= date('Y-m-d',strtotime($this->input->post('todate')));
					if ($chartofaccountID!=0) {
						$array['account']       	= $chartofaccountID;
					}else{
						$array['type']       	= 3;
					}
					if ($vouchertype!=0) {
						$array['vouchertype']       	= $vouchertype;
					} 
					 

					$this->data['expenses'] = $this->invoice_m->get_expense_report_by_array($array);
					 
					

					$retArray['render'] = $this->load->view('report/transaction/TransactionReport_expense',$this->data,true);
					$retArray['status'] = TRUE;
					echo json_encode($retArray);
					exit;
				}
	...
	public function pdf_expense() {
		if(permissionChecker('transactionreport')) {
			$fromdate = $this->uri->segment(3);
			$todate = $this->uri->segment(4);
			$pdfoption = $this->uri->segment(5);
			$chartofaccountID = $this->uri->segment(6);
			$vouchertype = $this->uri->segment(7);
			if((isset($fromdate) && (int)$fromdate) && (isset($todate) && (int)$todate) && (isset($pdfoption) && (int)$pdfoption)) {
				$this->data['fromdate'] = $fromdate;
				$this->data['todate'] = $todate;
				...
				$array = []; 
				$array['date  >='] = date('Y-m-d',$fromdate);
				$array['date  <='] = date('Y-m-d',$todate); 
				$this->data['pdfoption'] = $pdfoption;
				if ($chartofaccountID!=0) {
						$array['account']       	= $chartofaccountID;
					}else{
				...
				$this->reportPDF('transactionreport.css', $this->data, 'report/transaction/TransactionReportPDF_expense');
			} else {
				$this->data["subview"] = "error";
				$this->load->view('_layout_main', $this->data);		
			}
			...
			if(customCompute($get_payments)) {
				...
				foreach ($get_payments as $get_payment) {
					if(isset($weaverandfine[$get_payment->paymentID]) && (($weaverandfine[$get_payment->paymentID]->weaver != '') || ($weaverandfine[$get_payment->paymentID]->fine != '')) || $get_payment->paymentamount != '') {
                        ...
                        $bodys[$i][] = $get_payment->refrence_no;
  ...
  private function xmlData_expense() {
    $fromdate = $this->uri->segment(3);
    $todate = $this->uri->segment(4);
    $xmloption = $this->uri->segment(5);
    $chartofaccountID = $this->uri->segment(6);
    ...
    if((isset($fromdate) && (int)$fromdate) && (isset($todate) && (int)$todate) && (isset($xmloption) && (int)$xmloption)) {
      ...
      return $this->generateXML_expense($this->data);
    } else {
      $this->data["subview"] = "error";
      $this->load->view('_layout_main', $this->data);   
    }
  ...
  private function generateXML_expense($arrays) {
    extract($arrays);
    $sheet = $this->phpspreadsheet->spreadsheet->getActiveSheet();
    if($xmloption == 3 ) {
      if(customCompute($expenses)) {
        ...
        foreach ($expenses as $e) {
            ...
            $bodys[$i][] = $i+1;
            $bodys[$i][] = date('d M Y',strtotime($e->transaction_date));
            $bodys[$i][] = date('d M Y',strtotime($e->created_at));
            $bodys[$i][] = $e->description;
            $bodys[$i][] = $e->debit;
            $bodys[$i][] = $e->credit;
                      if($e->debit>0){
                            $debit+=$e->debit;
        ...
        $bodys[$i][] = "";
        $bodys[$i][] = ""; 
        $bodys[$i][] = number_format($debit,2);
        $bodys[$i][] = number_format($credit,2);
        $bodys[$i][] = number_format($balance,2);


        $fromdateValue = $this->lang->line('transactionreport_fromdate')." : ".date('d M Y',$fromdate);
        $todateValue   = $this->lang->line('transactionreport_todate')." : ".date('d M Y',$todate);
        $sheet->setCellValue('A1', $fromdateValue);
        $sheet->setCellValue('H1', $todateValue);
  ...
  public function income() {
    ...
    $this->data["subview"] = "report/transaction/TransactionReportView_income";
    $this->load->view('_layout_main', $this->data);
  }

  

  public function getTransactionReport_income() {
    $retArray['status'] = FALSE;
    $retArray['render'] = '';
    if(permissionChecker('transactionreport')) {
      if($_POST) {
        ...
        if ($this->form_validation->run() == FALSE) {
          ...
          $retArray['render'] = $this->load->view('report/transaction/TransactionReport_income',$this->data,true);
          $retArray['status'] = TRUE;
          echo json_encode($retArray);
          exit;
        }
  ...
  private function xmlData_income() {
    $fromdate = $this->uri->segment(3);
    $todate = $this->uri->segment(4);
    $xmloption = $this->uri->segment(5);
    $chartofaccountID = $this->uri->segment(6);
    ...
    if((isset($fromdate) && (int)$fromdate) && (isset($todate) && (int)$todate) && (isset($xmloption) && (int)$xmloption)) {
      ...
      return $this->generateXML_income($this->data);
    } else {
      $this->data["subview"] = "error";
      $this->load->view('_layout_main', $this->data);   
    }
  ...
  private function generateXML_income($arrays) {
    extract($arrays);
    $sheet = $this->phpspreadsheet->spreadsheet->getActiveSheet();
    if($xmloption == 3 ) {
      if(customCompute($expenses)) {
