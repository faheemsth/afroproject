        ...
        <div class="box" style="margin-bottom:40px">
            ...
            <div class="box-body">
                 <div class="dateDiv form-group <?=form_error('date') ? 'has-error' : '' ?>" >
                        <label for="date">
                            <?=$this->lang->line("invoice_pdate")?> <span class="text-red">*</span>
                        </label>
                ...
                <div class="form-group <?=form_error('paymentmethodID') ? 'has-error' : '' ?>">
                    <label for="paymentmethodID">Payment Type <span class="text-red">*</span></label>

                    <?php
                        $paymentmethodArray['0'] = $this->lang->line("invoice_select_paymentmethod");

                        if($this->session->userdata('usertypeID') == 1 || $this->session->userdata('usertypeID') == 5 || $this->session->userdata('usertypeID') == 14) {
                    ...
                    ?>
                        <label for="chart_account_id" class="control-label">
                        ...
                        <div>
                           <?php
                                ...
                                echo form_dropdown("chart_account_id", $array, set_value("chart_account_id"), "id='chart_account_id' class='form-control select2' ");
                            ?>
                        </div>
                ...
                <div class="table-responsive">
                    <table class="table table-bordered feetype-style" style="font-size: 16px;">
                        ...
                        <tbody id="feetypeList">
                            <?php 
                                ...
                                if(customCompute($invoices)) {
                                    // echo $maininvoices[0]->maininvoicenet_fee;
                                    // echo '<br>';
                                    // echo '<pre>';
                                    // print_r($maininvoices);
                                    // echo '</pre>';
                                    // echo '<br>';
                                    // echo '<pre>';
                                    // print_r($invoices);
                                    ...
                                    foreach ($invoices as $invoice) {
                                        ...
                                        // echo $invoice->net_fee;
                                        // echo '<br>';
                                        // $due = 0;
                                        // echo $invoice->invoiceID;
                                        // echo '<br>';
                                        // echo '<pre>';
                                        // print_r($invoicepaymentandweaver);
                                        // echo '</pre>';
                                        // echo '<br>';
                                        // if(isset($invoicepaymentandweaver['totalamount'][$invoice->invoiceID])) {
                                        //     echo $due = $invoicepaymentandweaver['totalamount'][$invoice->invoiceID]; 
                                        //     echo '<br>';
                                        //     if(isset($invoicepaymentandweaver['totaldiscount'][$invoice->invoiceID])) {
                                        //         echo $due =  ($due -$invoicepaymentandweaver['totaldiscount'][$invoice->invoiceID]);
                                        //         echo '<br>';
                                        //     }

                                        //     if(isset($invoicepaymentandweaver['totalpayment'][$invoice->invoiceID])) {
                                        //         echo $due = ($due - $invoicepaymentandweaver['totalpayment'][$invoice->invoiceID]);
                                        //         echo '<br>';
                                        //     } 

                                        //     if(isset($invoicepaymentandweaver['totalweaver'][$invoice->invoiceID])) {
                                        //         echo $due = ($due - $invoicepaymentandweaver['totalweaver'][$invoice->invoiceID]);
                                        //         echo '<br>';
                                        //     }
                                        // }
                                        ...
                                        $totalAmount += $amount; 
                                        // $totalDue += $due; 
                                        $due = $maininvoices[0]->maininvoicenet_fee;
                                        $totalDue = $maininvoices[0]->maininvoicenet_fee;
                                        ...
                                        echo '<tr id="tr_'.$rand.'">';
                                            ...
                                            if($this->session->userdata('usertypeID') == 1 || $this->session->userdata('usertypeID') == 5) {
                                                echo '<td>';
                                                    echo '<input readonly placeholder="edit invoice" id="weaver_'.$rand.'" class="form-control change-weaver '.(form_error('weaver_'.$invoice->invoiceID) ? 'bordered-red' : '').'" type="text" name="weaver_'.$invoice->invoiceID.'" value="'.set_value('weaver_'.$invoice->invoiceID).'" >';
                                                echo '</td>';

                                                echo '<td>';
                                                    echo '<input id="fine_'.$rand.'" class="form-control change-fine '.(form_error('fine_'.$invoice->invoiceID) ? 'bordered-red' : '').'" type="hidden" name="fine_'.$invoice->invoiceID.'" value="'.set_value('fine_'.$invoice->invoiceID).'" >';
                                                echo '</td>';
                                            }
                                        echo '</tr>';
                                        $i++;
                        ...
                        <tfoot id="feetypeListFooter">
                            <tr>
                                <td colspan="3" style="font-weight: bold"><?=$this->lang->line('invoice_total')?></td>
...
<script type="text/javascript">
    ...
    function totalInfo() {
        ...
        $('#feetypeList tr').each(function(index, value) {
            ...
            var trID = $(this).attr('id').replace('tr_','');
            ...
            if($(this).children().eq(6).children().val() != '' && $(this).children().eq(6).children().val() != null && $(this).children().eq(6).children().val() != '.') {
                var weaver = parseFloat($(this).children().eq(6).children().val());
                totalWeaver += weaver;
            }

            if($(this).children().eq(7).children().val() != '' && $(this).children().eq(7).children().val() != null && $(this).children().eq(7).children().val() != '.') {
                var fine = parseFloat($(this).children().eq(7).children().val());
                totalFine += fine;
            }
        });
        ...
        totalFine = totaldueAmount-totalPaidAmount-totalWeaver;
