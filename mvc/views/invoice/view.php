    ...
    <div class="well">
        <div class="row">
            ...
            <div class="col-sm-6">
                ...
                $maininvoice_ID = $this->uri->segment($secondLastKey);
                 echo btn_add_pdf('invoice/print_preview/'.$maininvoice_ID, $this->lang->line('pdf_preview')) 
                ?>
                    ...
                    <?php
                        if(permissionChecker('invoice_edit')) {
                            if($maininvoice->maininvoicestatus != 1 && $maininvoice->maininvoicestatus != 2) {
                                ...
                                echo btn_sm_edit('invoice/edit/'.$maininvoice_ID, $this->lang->line('edit'));
                            }
                        }
                    ?>
                <?php } ?>
                <?php
                    if($maininvoice->maininvoicestatus != 2  && permissionChecker('invoice_edit')) {
                        echo btn_payment('invoice/payment/'.$maininvoice_ID, $this->lang->line('payment')); 
                    }
                ?>
    		        ...
    		        <h2 class="page-header">
    		            ...
    		            <small class="pull-right"><?=
    		              //  $this->lang->line('invoice_create_date').' : '.date('d M Y');
    		                    $this->lang->line('invoice_date_from').' : '.$maininvoice->maininvoicedate.'<br><br>'.$this->lang->line('invoice_date_to').' : '.$maininvoice->maininvoicedue_date;
    		                ?>
    		            </small>
    		        </h2>
    		    </div><!-- /.col -->
    		...
    		<div class="row invoice-info">
    		    ...
    		    <div class="col-sm-4 invoice-col" style="font-size: 16px;">
    	        	...
    	        	<address >
    	        		...
    	        		<?php if(customCompute($student)) { echo 'Student ID'. " : ". $student->student_id; } ?><br>
    	        	</address>
    		    </div><!-- /.col -->
    		    <div class="col-sm-4 invoice-col" style="font-size: 16px;">
    		        <b><?=$this->lang->line("invoice_invoice").$maininvoice->refrence_no?></b><br>
                    <?php 
                ...
                <div class="col-xs-12">
                    <div class="table-responsive">
                        <table class="table table-bordered product-style">
                            ...
                            <tbody>
                                <?php 
                                    // $subtotal = 0; $totalsubtotal = 0; $i = 1; if(customCompute($invoices)) { foreach($invoices as $invoice) { $discount = 0; if($invoice->discount > 0) { $discount = (($invoice->amount/100) * $invoice->discount); } $subtotal = ($invoice->amount - $discount); $totalsubtotal += $subtotal; 
                                    ?>
                                    <tr>
                                        ...
                                        <td data-title="<?=$this->lang->line('invoice_feetype')?>">
                                            <!--<?=isset($feetypes[$maininvoice->feetypeID]) ? $feetypes[$invoice->feetypeID] : ''?>-->
                                            <?=$maininvoice->feetype; ?>
                                        </td>
                                        
                                        <td data-title="<?=$this->lang->line('invoice_amount')?>">
                                            <?=ceil($maininvoice->maininvoicetotal_fee-$maininvoice->totalfine)?>
                                        </td>

                                        <td data-title="<?=$this->lang->line('invoice_discount')?>">
                                            <?=ceil($maininvoice->maininvoice_discount)?>
                                        </td>
