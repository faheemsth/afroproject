        ...
        <div class="box">
            ...
            <div class="box-body">
                <form role="form" method="post" enctype="multipart/form-data" id="invoiceDataForm"> 
                    ...
                    <div class="classesDiv form-group <?=form_error('classesID') ? 'has-error' : '' ?>" >
                        <label for="classesID">
                            ...
                            <?php
                                ...
                                echo form_dropdown("classesID", $classesArray, set_value("classesID", $maininvoice->maininvoiceclassesID), "id='classesID' disabled class='form-control'");
                            ?>
                        <span class="text-red">
                    ...
                    <div class="studentDiv form-group <?=form_error('studentID') ? 'has-error' : '' ?>" >
                        <label for="studentID">
                            ...
                            <?php
                                ...
                                echo form_dropdown("studentID", $studentArray, set_value("studentID", $maininvoice->maininvoicestudentID), "id='studentID' disabled class='form-control'");
                            ?>
                        <span class="text-red">
                    ...
                    <div class="dateDiv form-group <?=form_error('date') ? 'has-error' : '' ?>" >
                     ...
                     <div class="dateDiv form-group <?=form_error('to_date') ? 'has-error' : '' ?>" >
                        <label for="date">
                            ...
                            <input type="text" class="form-control" id="to_date" name="to_date" value="<?=date('d-m-Y', strtotime($maininvoice->maininvoicedue_date))?>" >
                        <span class="text-red">
                    ...
                    <div class="statusDiv hide form-group <?=form_error('statusID') ? 'has-error' : '' ?>" >
                        <label for="statusID">
                    ...
                    <div class="remarksDiv form-group <?=form_error('remarks') ? 'has-error' : '' ?>" >
                        <label for="date">
                            Remarks <span class="text-red">*</span>
                        </label>
                            <input type="text" class="form-control" id="remarks" name="remarks" value="<?=set_value('remarks',$maininvoice->remarks)?>" required >
                        <span class="text-red">
                            <?php echo form_error('remarks'); ?>
                        </span>
                    </div>

                    <div class=" form-group <?=form_error('Decision') ? 'has-error' : '' ?>" >
                ...
                <div class="table-responsive">
                    <table class="table table-bordered feetype-style" style="font-size: 16px;">
                        ...
                        <tbody id="feetypeList">
                            <?php
                            $all_feebreakups    =   unserialize($maininvoice->fee_breakup);
                           // var_dump($all_feebreakups);
                                if(customCompute($all_feebreakups)) {
                                    ...
                                    foreach ($all_feebreakups as $invoice) {
                                        ...
                                        $discount = 0;
                                        if($invoice['fee_discount'] > 0) {
                                            $discount = ($invoice['fee_discount']);
                                            $totalDiscount += $invoice['fee_discount'];
                                        }

                                        // $subtotal = ($invoice->amount - $discount);
                                        $subtotal = $invoice['net_amount'];
                                        $totalAmount += $invoice['fee_amount'];
                                        ...
                                        echo '<input name="fee_amount[]" type="text" class="form-control change-amount" id="td_amount_id_'.$randID.'" data-amount-id="'.$randID.'" readonly value="'.$invoice['fee_amount'].'">';

                                        echo '<input type="hidden" class="form-control change-netamount" id="td_net_amount_id_'.$randID.'" data-netamount-id="'.$randID.'" value="'.($invoice['fee_discount']+$maininvoice->maininvoicenet_fee).'">';
                                        echo '</td>';

                                        echo '<td>';
                                        echo '<input  name="fee_discount[]"  type="text" class="form-control change-discount"   id="td_discount_id_'.$randID.'" data-discount-id="'.$randID.'" value="'.$invoice['fee_discount'].'">';
                                        echo '</td>';



                                        echo '<td>';
                                        echo  $invoice['fee_amount']-($invoice['fee_discount']+$maininvoice->maininvoicenet_fee) ;
                                        echo '</td>';

                                        echo '<td class="totalSubtotal_txt">';
                                        echo $maininvoice->maininvoicenet_fee;

                                        echo '<input name="feetypeID[]" type="hidden" class="form-control" id="feetypeID_'.$randID.'" data-amount-feetypeID="'.$randID.'" value="'.$invoice['feetypeID'].'">';
                                        echo '</td>';
                                        echo '</tr>';
                                        $i++;
                                    }
...
<script type="text/javascript">
    ...
    function totalInfo() {
        ...
        $('#feetypeList tr').each(function(index, value) {
            if($(this).children().eq(3).children().val() != '' && $(this).children().eq(3).children().val() != null && $(this).children().eq(3).children().val() != '.') {
                var amount = parseFloat($(this).children().eq(3).children().val());
                totalAmount += amount;
            } 
        });
        ...
        $('#feetypeList tr').each(function(index, value) {
            if($(this).children().eq(4).children().val() != '' && $(this).children().eq(4).children().val() != null && $(this).children().eq(4).children().val() != '.') {
               // var discount = parseFloat($(this).children().eq(3).children().val());
                 var discount = parseFloat($(this).find('input.change-discount').val());
        ...
        $('#feetypeList tr').each(function(index, value) {
            var netamount   = parseFloat($(this).find('input.change-netamount').val());
            var discount    = parseFloat($(this).find('input.change-discount').val());
            var subtotal    = 0;
            
           if (netamount < discount) {
                $('#editInvoiceButton').attr('disabled', 'disabled');
                subtotal = (netamount - discount);
                toastr["error"]('Please Enter Correct Amount. It must be less than or equal to '+netamount)
            toastr.options = {
                "closeButton": true,
                "debug": false,
                "newestOnTop": false,
            ...
            }else if(netamount > 0 && discount > 0) {
            ...
            $(this).children().eq(6).text(subtotal);
            //    alert('a-'+amount);
            // alert('d-'+discount);
            // alert('st-'+subtotal);
            totalSubtotal += subtotal;
       ...
       // $('.totalSubtotal_txt').text(currencyConvert(totalSubtotal));
    ...
    // $(document).on('change', '.change-discount', function() {
...
<script type="text/javascript">
    ...
    $(document).on('click', '#editInvoiceButton', function() {
        ...
        field.append('classesID', $('#classesID').val());
...
/*
        ...
        var feetypeitems = $('tr[id^=tr_]').map(function(){
            if($(this).children().eq(6).text() != '' && $(this).children().eq(6).text() != null) {
                totalsubtotal += parseFloat($(this).children().eq(6).text());
            }

            if($(this).children().eq(6).children().val() != '' && $(this).children().eq(6).children().val() != null) {
                totalpaidamount += parseFloat($(this).children().eq(6).children().val());
            }

            return { feetypeID : $(this).attr('invoicefeetypeid'), amount: $(this).children().eq(3).children().val(), discount : $(this).children().eq(4).children().val(), subtotal: $(this).children().eq(6).text() , paidamount: $(this).children().eq(7).children().val() };
        }).get();
